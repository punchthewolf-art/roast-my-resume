...
export default function ShareButton({ score, roastId }: ShareButtonProps) {
  ...
  const tweetText = `My resume just got ROASTED by AI and scored ${score}/10 🔥💀\n\nThink yours is better? Try it:\n${shareUrl}\n\n#RoastMyResume #Resume #JobSearch`;

  const linkedInText = `I just got my resume brutally roasted by AI and scored ${score}/10! 🔥\n\nCurious about your score? Try RoastMyResume:`;
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-3">
      ...
      <button
        ...
        className={`flex items-center gap-2 rounded-full border px-5 py-2.5 text-sm font-semibold transition-all hover:scale-105 ${
          copied
            ? "border-green-500/30 bg-green-500/10 text-green-400"
            : "border-card-border bg-card-bg text-foreground hover:bg-card-border"
        }`}
      >
        {copied ? (
          <>
